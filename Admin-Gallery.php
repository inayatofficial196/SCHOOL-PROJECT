...
<body>
    ...
    <nav>
        <ul>
            ...
            <li class="click"><a href="#"><i class="fa-solid fa-image"></i> Gallery</a></li>
    ...
    if (isset($_POST['Submit'])) {
        ...
        if (in_array($imageFileType, $allowedExtensions)) {
            ...
            $sql = "INSERT INTO gallery (image_name, image_path, upload_date) VALUES ('$imageName', '$imagePath', NOW())";
            if ($conn->query($sql) === TRUE) {
                echo "<script> alert('Image Uploaded Successfully!'); </script>";
                ?>
                <meta http-equiv = "refresh" content = "0; url= http://localhost/FYP/Admin-Gallery.php"/>
                <?php
            } else {
        ...
        <table rules="rows" frame="border">
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<td><a href='Gallery-Delete.php?id=$row[id]'>
                    <input type='submit' value='Delete' id='delete' onclick='return checkdelete()'></a></td>";
                    echo '</tr>';
                }
            } else {
                echo '<tr><td colspan="3">No Gallery Images Available.</td></tr>';
            }
            $conn->close();
            ?>
        </table>
